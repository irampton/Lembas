<template>
  <section class="mx-auto w-full max-w-5xl p-4 md:p-6" aria-label="All recipes">
    <div class="mb-4 flex items-center justify-between gap-3">
      <button type="button" class="rounded p-2 hover:bg-orange-100" :aria-expanded="showFilters" aria-controls="recipe-filters" aria-label="Filter recipes" @click="showFilters = !showFilters"><FunnelIcon class="size-5" aria-hidden="true" /></button>
      <div class="flex items-center">
        <select id="recipe-sort" v-model="sortBy" aria-label="Sort recipes by" class="appearance-none rounded-none border-0 border-b border-stone-400 bg-transparent px-2 py-1 focus:border-orange-700 focus:outline-none">
          <option value="createdAt">Date added</option><option value="title">Name</option>
        </select>
        <button type="button" class="rounded p-2 hover:bg-orange-100" :aria-label="`Sort ${descending ? 'ascending' : 'descending'}`" :title="descending ? 'Descending order' : 'Ascending order'" @click="descending = !descending">
          <svg class="h-5 w-4 text-orange-700" viewBox="0 0 16 20" stroke="currentColor" stroke-width="1.25" stroke-linejoin="round" aria-hidden="true">
    ...
    <div v-show="showFilters" id="recipe-filters" class="mb-4 flex flex-wrap gap-2" role="group" aria-label="Filter by cookbook">
      <button
        ...
        :key="cookbook.id"
        type="button"
        class="flex max-w-full items-center gap-1 rounded-full border px-3 py-1 text-sm transition-colors focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-orange-700"
        :class="isCookbookSelected(cookbook.id) ? 'border-orange-700 bg-orange-100 text-orange-900' : 'border-stone-300 bg-white text-stone-500'"
        :aria-pressed="isCookbookSelected(cookbook.id)"
        @click="toggleCookbook(cookbook.id)"
      >
        <CheckIcon v-if="isCookbookSelected(cookbook.id)" class="size-4 shrink-0" aria-hidden="true" />
        <span class="truncate">{{ cookbook.name }}</span>
      </button>
      <p v-if="!cookbooks.length" class="text-sm text-stone-500">No cookbooks available.</p>
    </div>
    ...
    <ul v-else-if="recipes.length" class="divide-y divide-stone-200">
      <li v-for="recipe in recipes" :key="recipe.id">
        <RecipeCard :recipe-id="recipe.id" :recipe-name="recipe.title" :ingredient-list="recipe.ingredients" :tags="recipe.tags" />
      </li>
    </ul>
    <p v-else-if="!store.state.error" role="status">{{ store.state.recipes.length ? 'No recipes match your filters.' : 'No recipes yet.' }} <RouterLink v-if="!store.state.recipes.length" :to="{ name: 'recipe-new' }" class="text-orange-700 underline">Create a recipe</RouterLink></p>
  </section>
</template>
<script setup>
